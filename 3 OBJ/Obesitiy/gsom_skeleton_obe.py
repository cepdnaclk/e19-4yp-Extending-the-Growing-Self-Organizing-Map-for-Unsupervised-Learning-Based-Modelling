...
#     ax.set_title(figure_label)
#     ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=6)
#     plt.savefig("gsom_labels_obe.png")
#     plt.savefig(file_name + file_type, bbox_inches='tight')
#     plt.show()
def plot_labels(output, index_col, gsom_map=None, file_name="gsom", file_type=".pdf", 
         figure_label="GSOM Map with Paths", max_text=1, max_length=30, 
    ...
    fig, ax = plt.subplots(figsize=(10, 8))
    label_to_color = {}
    label_colors = cm.get_cmap("tab10", output[index_col].explode().nunique())
    label_handles = {}
    ...
    unique_labels = pd.unique(output[index_col].explode())
    for idx, label in enumerate(unique_labels):
        label_to_color[label] = label_colors(idx)

    for index, i in output.iterrows():
        x = i['x']
        y = i['y']
        node_labels = i[index_col]
        if isinstance(node_labels, list) and node_labels:
            label = node_labels[0]
        else:
            label = node_labels
        color = label_to_color[label]
        handle_label = str(label)
        # Only add one handle per label for the legend
        if handle_label not in label_handles:
            label_handles[handle_label] = ax.plot(x, y, 'o', color=color, markersize=6, label=handle_label)[0]
        else:
            ax.plot(x, y, 'o', color=color, markersize=6)
        # if show_index and i['hit_count'] > 0:
        #     text_label = ", ".join(map(str, i[index_col][0:max_text]))
        #     txt = ax.text(x, y, text_label, ha='left', va='center', wrap=True, fontsize=8)
        #     txt._get_wrap_line_width = lambda: max_length

    ax.set_title(figure_label)
    ax.legend(handles=list(label_handles.values()), bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=6, title="Labels")
    plt.savefig("gsom_labels_obe.png")
    plt.savefig(file_name + file_type, bbox_inches='tight')
    plt.show()
...
if __name__ == '__main__':
    ...
    plot_labels(output, "NObeyesdad", gsom_map=gsom, file_name="gsom_with_labels_obe",
             file_type=".pdf", figure_label="GSOM Map with Node Paths", n_nodes=gsom.node_count)
