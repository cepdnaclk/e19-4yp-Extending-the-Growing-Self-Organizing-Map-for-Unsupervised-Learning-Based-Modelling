...
class GSOM:
    ...
    def separate_clusters(self, data):
        """
        Automatically separate clusters using silhouette score to determine the optimal number of clusters.
        """
        ...
        # Sort segments by distance (descending)
        ...
        # Use node coordinates for silhouette score
        X = self.node_coordinate[:self.node_count]
        
        # Initial clustering (before removing any edges)
        initial_clusters = list(nx.connected_components(G))
        if len(initial_clusters) > 1:
            labels = np.zeros(self.node_count, dtype=int)
            ...
            try:
                ...
                print(f"Initial clusters: {len(initial_clusters)}, Silhouette Score: {score:.4f}")
            except ValueError:
                silhouette_scores.append(-1)  # Handle case where silhouette score fails
                clusters.append(initial_clusters)
        
        # Iteratively remove edges and compute silhouette score
        for i, j, dist in segments:
            ...
            if G.has_edge(i, j):
                ...
                # Only compute silhouette score if there are multiple clusters
                if len(current_clusters) > 1:
                    ...
                    try:
                        ...
                        silhouette_scores.append(score)
                        print(f"Removed segment {i}-{j}, Distance: {dist:.4f}, Clusters: {len(current_clusters)}, Silhouette Score: {score:.4f}")
                    except ValueError:
                        silhouette_scores.append(-1)  # Handle cases where silhouette score fails
                        clusters.append(current_clusters)
                else:
                    clusters.append(current_clusters)
                    silhouette_scores.append(-1)
                    print(f"Removed segment {i}-{j}, Distance: {dist:.4f}, Clusters: {len(current_clusters)}, Silhouette Score: Not computed (single cluster)")
        
        # Select the clustering with the highest silhouette score
        if silhouette_scores and max(silhouette_scores) > -1:
            best_idx = np.argmax(silhouette_scores)
            print(f"Best silhouette score found at index {best_idx}: {silhouette_scores[best_idx]:.4f}")
            best_clusters = clusters[best_idx]
            print(f"Selected {len(best_clusters)} clusters with Silhouette Score: {silhouette_scores[best_idx]:.4f}")
        else:
            best_clusters = clusters[-1]  # Fallback to the last clustering
            print(f"No valid silhouette score, using final clustering with {len(best_clusters)} clusters")
        
        return [best_clusters], segments, remaining_connections, pos_edges
...
if __name__ == '__main__':
    ...
    clusters, segments, skeleton_connections, pos_edges = gsom.separate_clusters(data_training.to_numpy())
