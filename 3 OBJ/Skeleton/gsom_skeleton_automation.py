...
class GSOM:
    ...
    def separate_clusters(self, data, threshold_factor=1.0):
        """
        Automatically separate clusters without specifying max_clusters.
        Uses a dynamic distance threshold based on segment distances.
        """
        ...
        if segments:
            ...
            threshold = 0  # Fallback if no segments
        
        G = nx.Graph(skeleton_connections)
        clusters = []
        remaining_connections = skeleton_connections.copy()
        
        # Remove edges until distance is below threshold
        for i, j, dist in segments:
            if dist < threshold:
                break  # Stop splitting when distance is below threshold
        ...
        # If no clusters formed, use initial connected components
        if not clusters:
            clusters.append(list(nx.connected_components(G)))
...
if __name__ == '__main__':
    ...
    clusters, segments, skeleton_connections, pos_edges = gsom.separate_clusters(data_training.to_numpy(), threshold_factor=1.0)
