...
# GSOM class (modified only for max_clusters)
class GSOM:
    ...
    def compute_cluster_purity(self, data, label_col, weight_columns):
        ...
        clusters, _, _, _ = self.separate_clusters(data, weight_columns, max_clusters=2)
...
def plot_analysis(gsom, output, entropy_dict, clusters, node_to_cluster, outliers, region_entropy, 
                 ...
                 cluster_names, file_name="gsom_boundary_analysis_adult.pdf"):
    ...
    if boundary_points:
        ...
        ax3.set_xticklabels(top_features, rotation=45)
...
if __name__ == '__main__':
    ...
    # Combine features and target, add Id column
    df = X.copy()
    df['income'] = y['income']
    ...
    # Define feature columns (after encoding)
    weight_columns = [col for col in df_encoded.columns if col not in ['Id', 'income']]
    
    # Normalize features
    scaler = MinMaxScaler()
    df_encoded[weight_columns] = scaler.fit_transform(df_encoded[weight_columns])
    
    # Clean income labels (handle variations like '<=50K.')
    df_encoded['income'] = df_encoded['income'].str.replace('.', '', regex=False)
    
    print("Dataset shape:", df_encoded.shape)
    data_training = df_encoded[weight_columns]
    print("Training data head:", data_training.head())
    print("Training data shape:", data_training.shape)
    
    # Train GSOM
    gsom = GSOM(spred_factor=0.83, dimensions=len(weight_columns), max_radius=6, initial_node_size=50000)
    gsom.fit(data_training.to_numpy(), 100, 50)
    output = gsom.predict(df_encoded, "Id", "income")
    output.to_csv("output_adult.csv", index=False)
    print("GSOM training completed.")
    print("Output shape:", output.shape)
    print("Node Count:", gsom.node_count)
    
    # Compute cluster purity
    entropy_dict, confusion, all_labels, node_to_cluster, cluster_names = gsom.compute_cluster_purity(df_encoded, "income", weight_columns)
    print("Node Entropy:")
    ...
    # Detect outliers
    outliers = gsom.detect_outliers(df_encoded, "income", weight_columns, threshold=2.0)
    print("\nOutliers:")
    ...
    region_entropy, region_nodes, deviant_points = gsom.analyze_region(center_node, radius=2.0, data=df_encoded, label_col="income", weight_columns=weight_columns)
    ...
    boundary_points, boundary_nodes, node_to_cluster, clusters = gsom.identify_boundary_points(df_encoded, weight_columns, "income", max_clusters=2, distance_threshold=0.3)
    print("\nBoundary Points:")
    
    # Save boundary points analysis
    with open("boundary_points_analysis_adult.txt", "w") as f:
        ...
        for point in boundary_points:
            ...
            for i, diffs in enumerate(point['feature_diff_other_clusters']):
                f.write(f"    Other Cluster {i+1}:\n")
                for feature, diff in zip(weight_columns, diffs):
                    f.write(f"      {feature}: {diff:.4f}\n")
            
            f.write(f"  Distances to Cluster Centroids: {[f'{d:.4f}' for d in point['distances_to_centroids']]}\n")
            f.write("-" * 60 + "\n")
    
    print(f"\nBoundary points analysis saved to 'boundary_points_analysis_adult.txt'")
    
    # Save comprehensive analysis report
    with open("gsom_complete_analysis_adult.txt", "w") as f:
        f.write("GSOM Complete Analysis Report\n")
        f.write("=" * 50 + "\n\n")
        
        f.write("DATASET INFORMATION\n")
        f.write("-" * 20 + "\n")
        f.write(f"Dataset shape: {df_encoded.shape}\n")
        ...
        for deviant in deviant_points:
            f.write(f"  Node {deviant['node']}, Sample ID: {deviant['sample_id']}, Label: {deviant['label']}, Distance: {deviant['distance']:.4f}\n")
        f.write("\n")
    ...
    print(f"Complete analysis report saved to 'gsom_complete_analysis_adult.txt'")
    
    # Visualize results
    plot_analysis(gsom, output, entropy_dict, clusters, node_to_cluster, outliers, region_entropy, 
                  region_nodes, deviant_points, boundary_points, boundary_nodes, df_encoded, "income", weight_columns, cluster_names)
