...
class GSOM:
    ...
    def compute_quantize_difference(self, data, weight_columns, label_col):
        """
...
def plot_analysis(gsom, output, entropy_dict, clusters, node_to_cluster, outliers, region_entropy, 
                 ...
                 file_name="gsom_regional_analysis.pdf"):
    """
    ...
    # Main GSOM map with regional analysis
    ax1 = fig.add_subplot(gs[0, :2])
    # Confusion matrix
    ax2 = fig.add_subplot(gs[0, 2])
    # Feature differences for boundary points
    ax3 = fig.add_subplot(gs[1, :])
    # Regional purity analysis
    ...
    # Enhanced confusion matrix with regional context
    ...
    bars = ax4.bar(range(len(purities)), purities, color=colors_purity, alpha=0.7)
    ax4.set_xlabel("Region ID")
    ax4.set_ylabel("Purity Score")
    ax4.set_title("Regional Purity Analysis\n(Green=Pure, Red=Mixed)")
    ax4.set_ylim(0, 1)
    ...
    if quantize_differences:
        ...
        ax5.bar(range(len(qd_values)), qd_values, color=colors_qd, alpha=0.7)
        ax5.set_xlabel("Node ID")
        ax5.set_ylabel("Quantize Difference")
        ax5.set_title("Quantize Difference by Node\n(Red=Mixed, Green=Pure)")
    ...
    bars_outliers = ax6.bar(outlier_types, outlier_counts, color=colors_outliers, alpha=0.7)
    ax6.set_ylabel("Number of Outliers")
    ax6.set_title("Outlier Analysis Summary\nBy Type")
...
if __name__ == '__main__':
    ...
    dataset_configs = {
        ...
        # Add 'adult' configuration when schema is provided
    }
    
    # Select dataset (change to 'zoo' or 'adult' as needed)
    dataset_name = 'iris'  # or 'iris' 'zoo' or 'adult'
    ...
    quantize_differences = gsom.compute_quantize_difference(df, config['weight_columns'], config['label_col'])
    ...
    # Enhanced outlier detection with explanations
    outliers = gsom.detect_outliers(df, config['label_col'], config['weight_columns'], threshold=2.0)
    ...
    region_entropy, region_nodes, deviant_points = gsom.analyze_region(
        center_node, radius=2.0, data=df, label_col=config['label_col'], weight_columns=config['weight_columns']
    )
    ...
    with open(boundary_file, "w") as f:
        ...
        for explanation, points in explanation_groups.items():
            ...
            for point in points:
                cluster_name = cluster_names.get(point['cluster'], f"Region {point['cluster'] + 1}")
                animal_name = df.loc[df['Id'] == point['sample_id'], 'animal_name'].iloc[0] if dataset_name == 'zoo' else point['sample_id']
    ...
    plot_file = f"gsom_regional_analysis_{dataset_name}.pdf"
    plot_analysis(gsom, output, entropy_dict, clusters, node_to_cluster, outliers, region_entropy, 
                  ...
                  quantize_differences, plot_file)
