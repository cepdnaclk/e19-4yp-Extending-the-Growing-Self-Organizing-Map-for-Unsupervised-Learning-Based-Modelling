...
class GSOM:
    ...
    def analyze_idionomic_features(self, data, index_col, label_col=None):
        weight_columns = list(data.columns.values)
        if label_col:
            weight_columns.remove(label_col)
        weight_columns.remove(index_col)
        ...
        out = scipy.spatial.distance.cdist(self.node_list[:self.node_count], data_n, self.distance)
        winner_indices = out.argmin(axis=0)
        print("Winner indices:", winner_indices)
        
        idionomic_data = []
        for idx, (name,row) in enumerate(data[[index_col]].iterrows()):
            node_idx = winner_indices[idx]
            node_weights = self.node_list[node_idx]
            data_point = data_n[idx]
            feature_diff = np.abs(data_point - node_weights)
            sum_diff = np.sum(feature_diff)
            if sum_diff == 0:
                feature_importance = np.ones_like(feature_diff) / len(feature_diff)
            else:
                feature_importance = feature_diff / sum_diff
...
def plot_dsm(gsom, output, clusters, segments, pos_edges, idionomic_df, feature_names, index_col, label_col=None, 
             file_name="gsom_dsm", file_type=".pdf", figure_label="GSOM Data Skeleton Model with Idionomic Features"):
    max_count = output["hit_count"].max()
    listed_color_map = colors.ListedColormap(
        plt.get_cmap("Paired", max_count + 1)(np.arange(max_count + 1)) * 0.9, 
...
def plot_idionomic_heatmap(idionomic_df, feature_names, file_name="idionomic_heatmap", file_type=".pdf"):
    ...
    sns.heatmap(importance_matrix, xticklabels=feature_names, yticklabels=idionomic_df['name'],
                cmap='viridis', cbar_kws={'label': 'Feature Importance'})
    ax.set_title("Idionomic Feature Importance Heatmap")
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig(file_name + file_type, bbox_inches='tight')
    plt.close()

def plot_node_feature_importance(idionomic_df, output, feature_names, file_dir="node_feature_plots", file_type=".png"):
...
if __name__ == '__main__':
    ...
    idionomic_df, feature_names = gsom.analyze_idionomic_features(df, index_col="Name", label_col="label")
    ...
    plot_dsm(gsom, output, clusters, segments, pos_edges, idionomic_df, feature_names,
             ...
             figure_label="GSOM Data Skeleton Model with Idionomic Features")
    ...
    print("DSM and all idionomic visualizations complete.")