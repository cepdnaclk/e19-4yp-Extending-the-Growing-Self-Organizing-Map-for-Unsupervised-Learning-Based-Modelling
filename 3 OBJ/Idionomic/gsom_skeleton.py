...
class GSOM:
    ...
    def build_skeleton(self, data):
        hit_points, skeleton_connections, junctions, pos_edges = super().build_skeleton(data)
...
if __name__ == '__main__':
    ...
    hit_points, skeleton_connections, junctions, pos_edges, individual_mapping = gsom.build_skeleton(data_training.to_numpy())
    # Plot GSOM map
...
#################################
    ...
    for node_idx, data_indices in individual_mapping.items():
        ...
        ax.scatter(x, y, c=f'C{len(data_indices) % 10}', s=100, label=f'Node {node_idx} (Individuals: {len(data_indices)})')
