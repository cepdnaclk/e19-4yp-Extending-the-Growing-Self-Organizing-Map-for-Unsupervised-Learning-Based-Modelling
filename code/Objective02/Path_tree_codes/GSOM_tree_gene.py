...
def generate_filtered_gsom_tree(gsom, output_csv="output.csv", output_path="filtered_tree.png", label_col="Sample_ID"):
    """
    Generate and export a GSOM path tree that only includes nodes with original data mappings.
    Works with bigtree structure. Now supports dynamic label column.
    """
    import pandas as pd
    from bigtree import Node, tree_to_dot
    ...
    if 'output' not in output_df.columns or label_col not in output_df.columns:
        raise ValueError(f"Required columns 'output' and '{label_col}' not found in output CSV.")

    data_nodes = set(output_df['output'])
    label_map = output_df.set_index('output')[label_col].to_dict()
    ...
    def clone_relevant_nodes(node):
        ...
        if new_node.name == "root" or (new_node.children or node.name.isdigit() and int(node.name) in data_nodes):
            return new_node
        return None
...
if __name__ == '__main__':
    ...
    output_folder = "output_gene"
    os.makedirs(output_folder, exist_ok=True)

    # Load gene expression data
    data_filename = "example/data/GSE/GSE5281_merged_expression_metadata.csv".replace('\\', '/')
    df = pd.read_csv(data_filename)
    print("Dataset shape:", df.shape)

    # Extract only expression data for training (exclude metadata)
    data_training = df.iloc[:, 3:]
    print("Training data shape:", data_training.shape)

    # Train GSOM
    gsom = GSOM(0.83, data_training.shape[1], max_radius=4, initial_node_size=1000)
    gsom.fit(data_training.to_numpy(), 100, 50)

    # Predict using full DataFrame with metadata
    ...
    # GSOM node map plots
    plot(output, "Sample_ID", gsom_map=gsom, file_name=os.path.join(output_folder, "gsom_map"))
    plot_pos(output, "Sample_ID", gsom_map=gsom, file_name=os.path.join(output_folder, "gsom_with_paths_sk"),
    ...
    for i, j in skeleton_connections:
        color, alpha, style = ('black', 0.5, '-') if (i, j) in pos_edges or (j, i) in pos_edges else ('red', 0.1, '-')
        if overlaps[(i, j)] < q3:
            color, alpha, style = 'gray', 0.3, '--'
        x1, y1 = gsom.node_coordinate[i]
        x2, y2 = gsom.node_coordinate[j]
        ax.plot([x1, x2], [y1, y2], color=color, linestyle=style, alpha=alpha)
    ...
    # Draw clusters
    colors_list = ['green', 'red', 'black', 'cyan', 'orange', 'magenta']
    clusters_data = []
    ...
    clusters_df.to_csv(os.path.join(output_folder, "clusters_gene.csv"), index=False)
    ...
    # Tree visualization
    tree_to_dot(gsom.path_tree).write_png(os.path.join(output_folder, "path_tree.png"))
    generate_filtered_gsom_tree(
        gsom,
        output_csv=os.path.join(output_folder, "output.csv"),
        output_path=os.path.join(output_folder, "filtered_tree.png"),
        label_col="Sample_ID"  # Use "Name" or "index" if you're using Zoo/Iris datasets
    )
